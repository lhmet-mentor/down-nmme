...
8. `aggregate-members.R`: 
  ...
  - depende do script `aggregate-nmme.R` que tem as funções abaixo:
    ...
    - `join_nmme_models_members_ensemble()`: juntar os arquivos de saída das funções `join_nmme_model_ensemble()` e `spread_members_nmme()` formando o arquivo de dados principal; ou seja com as previsões de cada membro, a média por modelo e a média do conjunto. Se `climatology = TRUE` (default) a previsão baseada na cliamtologia é inlcuída também.
...
XX. Métodos de previsão univariada
